...
env = RandomMiniGridEnv(env_ids=env_ids, max_len=max_len, frame_num=recurrent_frame_num, scale=0, render_human=True)
env = MissionToArrayWrapper(env, tokenizer, mission_max_length, recurrent_frame_num*3)

model = RecurrentPPO.load(f"model/save_model/model_RecurrentPPO_{test_learning_steps}.zip", env=env, device='cuda')  # 또는 'cpu'
