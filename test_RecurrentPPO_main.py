...
env = RandomMiniGridEnv(env_ids=["BabyAI-SynthS5R2-v0"], max_len=max_len, frame_num=recurrent_frame_num, scale=scale, render_human=True)
env = MissionToArrayWrapper(env, tokenizer, mission_max_length, recurrent_frame_num*3)

model = RecurrentPPO.load(f"model/save_model/8x8_model_RecurrentPPO_{test_learning_steps}_9.zip", env=env, device='cuda')  # 또는 'cpu'
